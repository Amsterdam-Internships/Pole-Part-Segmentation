...
def inference_caller(path, step_ind, on_val):
    ...
    if config.dataset.startswith('ShapeNetPart'):
        dataset = InferenceDataset(config.dataset.split('_')[1], config.input_threads)
    ...
    dataset.init_inf_input_pipeline(config)
    ...
    inferer = ModelInference(model, restore_snap = chosen_snap)
    t2 = time.time()
    ...
    if config.dataset.startswith('ShapeNetPart'):
        ...
        #     tester._multi_segmentation(model, dataset)
       
        inferer.infer_segmentation(model, dataset)
    # elif config.dataset.startswith('S3DIS'):
...
if __name__ == '__main__':
    ...
    chosen_log = '/home/s2478366/internship_repo/KPConv_Experiment/results/Log_2022-02-27_17-51-18'
    ...
    if chosen_log in handled_logs:
        ...
        # Dataset name
        infer_dataset = '_'.join(chosen_log.split('_')[1:])
        ...
        for log in logs[::-1]:
            log_config = Config()
            log_config.load(log)
            if log_config.dataset.startswith(infer_dataset):
                chosen_log = log
                break

        if chosen_log in handled_logs:
            raise ValueError('No log of the dataset "' + infer_dataset + '" found')
    ...
    # Let's go
    inference_caller(chosen_log, chosen_snapshot, on_val)

...
#        self.name_to_label = {v: k for k, v in self.label_to_names.items()}
    ...
    def load_evaluation_points(self, file_path):
        ...
        if color_info and intensity_info:
            return np.vstack((data['x'], data['y'], data['z'], data['red'], data['green'], data['blue'], data['intesnity'])).T
        elif color_info:
    ...
    def calibrate_batches(self, config):
        if 'cloud' in self.network_model:
            ...
            if len(self.input_trees['training']) > 0:
                split = 'training'
            elif len(self.input_trees['test']) > 0:
                split = 'test'
            elif en(self.input_trees['validation']) > 0:
                split = 'validation'
            else:
            	print('\nError(1) in data split\n')
            ...
            if len(self.input_points['training']) > 0:
                split = 'training'
            elif len(self.input_points['test']) > 0:
                split = 'test'
            elif en(self.input_points['validation']) > 0:
                split = 'validation'
            else:
                print('\nError(2) in data split\n')
    ...
    def calibrate_neighbors(self, config, keep_ratio=0.8, samples_threshold=10000):
        ...
        if 'cloud' in self.network_model:
            ...
            if len(self.input_trees['training']) > 0:
                ...
                split = 'test'
            elif len(self.input_trees['validation']) > 0:
                split = 'validation'
            else:
                print('\n Error(1) in Calibrate neighbors\n')
            ...
            if len(self.input_points['training']) > 0:
                ...
                print('\nError(2) in Calibrate neighbors\n')
    ...
    def init_input_pipeline(self, config):
        ...
        # Validation dataset
        ##############
        gen_function_val, gen_types, gen_shapes = self.get_batch_gen('validation', config)
        # Create batched dataset from generator
        ...
        #iter2 = tf.data.Iterator.from_structure(self.val_data.output_types, self.val_data.output_shapes)
        #self.flat_inputs = iter2.get_next()

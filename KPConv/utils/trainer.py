...
class ModelTrainer:
    ...
    def train(self, model, dataset, debug_NaN=False):
        ...
        if model.config.saving:
            ...
            with open(join(model.saving_path, 'validation.txt'), "w") as file:
            	file.write('Epochs steps out_loss validation_accuracy time\n')
    ...
    def segment_validation_error(self, model, dataset):
        ...
        mean_dt = np.zeros(2)
        last_display = time.time()
        print(model.config.validation_size)
        for i0 in range(model.config.validation_size):
#            print('\n******************\n',i0)
            try:
                # Run one step of the model.
                t = [time.time()]
                ops = [self.prob_logits, model.labels, model.output_loss,self.accuracy, model.inputs['in_batches'], model.inputs['object_inds']]
                print(ops)
                prob, labels, L_out, acc, batches, o_inds= self.sess.run(ops, {model.dropout_prob: 1.0})
                t += [time.time()]
                if model.config.saving:
                	process = psutil.Process(os.getpid())
 #               	print('\n*********next**********\n',i0)
...
#                                                          acc))
        ...
        #wandb.log({'IoU': IoUs, 'vote_IoUs': vote_IoUs})
        # Saving (optionnal)
        if model.config.saving:

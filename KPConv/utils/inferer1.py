...
class ModelInference:
    ...
    def infer_segmentation(self, model, dataset):
        ...
        original_path = join(dataset.path, 'inf_ply')
        ...
        if model.config.saving:
            infer_path = join('inference', model.saving_path.split('/')[-1])
        ...
        while True:
            try:
                # Run one step of the model
                t = [time.time()]
                ops = (self.prob_logits,model.inputs['in_batches'],model.inputs['points'],model.inputs['augment_scales'],model.inputs['augment_rotations'])
            ...
            #     accm = np.add(accm, confusion_matrix(original_labels[i], np.argmax(average_predictions[i], axis=1), parts))
            #     if v==1:
            #     	obj_path = join(test_path, object_name)
            #     	if not exists(obj_path):
            #     		makedirs(obj_path)
            #     	confusion_file = join(obj_path, 'confusion.txt')
            #     	with open(confusion_file, "w") as text_file:
            #     		text_file.write('accumulated confusion matrix over all test poles : \n'+ str(accm))
            #     #print('Confusion Matrix per object part for the cloud number ', i, '\n')
            #     #print(confusion_matrix(original_labels[i], np.argmax(average_predictions[i], axis=1), parts))
            #     Confs += [confusion_matrix(original_labels[i], np.argmax(average_predictions[i], axis=1), parts)]
            #
            # t2 = time.time()
            # print('Done in {:.1f} s\n'.format(t2 - t1))
            # #print('accumulated confusion matrix over all test poles : \n', accum_conf)
            #
            ...
            # #print('Confusion Matrix Stacked per object class for each cloud: \n')
            # #print(Confs)
            ...
            # print('Objs | Inst | Air  Bag  Cap  Car  Cha  Ear  Gui  Kni  Lam  Lap  Mot  Mug  Pis  Roc  Ska  Tab   Pole')
            # print('-----|------|---------------------------------------------------------------------------------------')
            #
            # s = '---- | ---- | '
